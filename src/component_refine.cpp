...
namespace weavess {
    ...
    void ComponentRefineNSG::RefineInner() {
        ...
        for (size_t i = 0; i < index->getBaseLen(); i++) {
            ...
            index->getFinalGraph()[i][1].resize(pool_size);
            for (unsigned j = 0; j < pool_size; j++) {
                index->getFinalGraph()[i][1][j] = pool[j].id;
            }
        }
    }

    void ComponentRefineNSG::SetConfigs() {
        index->setEntryType(ENTRY_NSG_CENTROID);
        index->setCandidateType(CANDIDATE_NAIVE);
        index->setPruneType(PRUNE_NSG);
...
#pragma omp parallel
        {
            ...
            if (index->getCandidateType() == CANDIDATE_NAIVE) {
                std::cout << "__CANDIDATE : NAIVE__";
                a = new ComponentCandidateNSG(index);
            }

            // PRUNE
            ComponentPrune *b = nullptr;

            if (index->getPruneType() == PRUNE_NSG) {
                std::cout << "__PRUNE : NSG__";
                b = new ComponentPruneNSG(index);
            }
...
#pragma omp for schedule(dynamic, 100)
            for (unsigned n = 0; n < index->getBaseLen(); ++n) {
                ...
                a->CandidateInner(n, index->ep_, visited_list, pool, 0);
...
#pragma omp for schedule(dynamic, 100)
    ...
    void ComponentRefineNSSG::SetConfigs() {
        index->setEntryType(ENTRY_NSSG_CENTROID);
        index->setCandidateType(CANDIDATE_NAIVE);
        index->setPruneType(PRUNE_NSSG);

        index->R_nsg = index->getParam().get<unsigned>("R_nsg");
        index->L_nsg = index->getParam().get<unsigned>("L_nsg");
        index->A = index->getParam().get<unsigned>("A");
...
#pragma omp parallel
        {
            ...
            ComponentPrune *b = nullptr;
            if (index->getPruneType() == PRUNE_NSSG)
                b = new ComponentPruneNSSG(index);
...
#pragma omp for schedule(dynamic, 100)
    ...
    void ComponentRefineDPG::Link(Index::SimpleNeighbor *cut_graph_) {
        ...
        ComponentPrune *a = nullptr;

        if (index->getPruneType() == PRUNE_DPG) {
            a = new ComponentPruneDPG(index);
        }

        ComponentCandidateNone *b = new ComponentCandidateNone(index);

#pragma omp parallel
        {
#pragma omp for schedule(dynamic, 100)
            for (unsigned n = 0; n < index->getBaseLen(); ++n) {
                b->CandidateInner(n, 0, visited_list, pool, 0);
                a->PruneInner(n, index->L_dpg, visited_list, pool, cut_graph_, 0);
            }
        }
//
//#pragma omp for schedule(dynamic, 100)
//        for (unsigned n = 0; n < index->getBaseLen(); ++n) {
////            InterInsert(n, index->L_dpg, locks);
//        }
//    }
    }
    ...
    void ComponentRefineEFANNA::RefineInner() {
        ...
        std::cout << index->getBaseLen() << std::endl;
        unsigned K = index->K;
...
//        }
    ...
    int ComponentRefineHNSW::SetConfigs() {
        ...
        index->setPruneType(PRUNE_HEURISTIC);
    }
...
//                    }
        ...
        if (index->getPruneType() == PRUNE_NAIVE) {
            a = new ComponentPruneNaive(index);
        } else if (index->getPruneType() == PRUNE_HEURISTIC) {
        ...
        if (index->getPruneType() == PRUNE_NAIVE) {
            ...
            for (size_t i = 1; i < neighbors.size(); ++i) {
                float curd = index->getDist()->compare(index->getBaseData() + source->GetId() * index->getBaseDim(),
                                                       index->getBaseData() +
                                                       neighbors[i]->GetId() * index->getBaseDim(),
                                                       index->getBaseDim());
                if (curd > max) {
                    max = curd;
                    maxi = i;
...
#pragma omp parallel
        {
            ...
            for (unsigned t = 0; t < 2; t++) {
                ...
                std::vector<std::vector<Index::Neighbor>> temp_graph;
                std::vector<std::vector<bool>> visited(index->getBaseLen(),
                                                       std::vector<bool>(index->getBaseLen(), false));
                temp_graph.resize(index->getBaseLen());
